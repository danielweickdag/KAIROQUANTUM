...
class PricingWorkflowService {
  private triggers: WorkflowTrigger[] = [
    ...
    ,
    {
      id: 'creator-following-growth',
      name: 'Creator Program Candidate',
      condition: (metrics) => metrics.creatorsFollowed >= 5 || metrics.copyTradesCount >= 25,
      action: 'suggest_upgrade',
      targetPlan: 'creator-annual',
      message: 'You are engaging heavily with creators. Consider the Creator Annual plan (variable per creator).',
      priority: 'medium'
    },
    {
      id: 'creator-activity-quarterly',
      name: 'Creator Program Quarterly Option',
      condition: (metrics) => metrics.creatorsFollowed >= 3 && metrics.monthlyVolume > 10000,
      action: 'suggest_upgrade',
      targetPlan: 'creator-quarterly',
      message: 'Quarterly Creator Program option available with flexible pricing per creator.',
      priority: 'low'
    }
  ]
