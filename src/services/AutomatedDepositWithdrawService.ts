...
class AutomatedDepositWithdrawService {
  ...
  /**
   * Return connected (verified) bank accounts for the current user
   */
  async getConnectedBankAccounts(): Promise<BankAccount[]> {
    const accounts = await bankAccountIntegrationService.getBankAccounts();
    return accounts.filter(acc => acc.isVerified);
  }
