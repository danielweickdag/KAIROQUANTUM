...
class BankAccountService {
  ...
  async disconnectAccount(accountId: string): Promise<boolean> {
    try {
      return await bankAccountIntegrationService.removeBankAccount(accountId)
    } catch (error) {
   ...
   */
  async verifyAccount(accountId: string, verificationData: number[]): Promise<boolean> {
    try {
  ...
  async getAccountTransactions(accountId: string, limit: number = 50): Promise<any[]> {
    try {
      return await bankAccountIntegrationService.getTransactionHistory(limit)
    } catch (error) {
