...
class AlertService {
  ...
  private loadUserPreferences() {
    try {
      ...
      const stored = canUseStorage ? window.localStorage.getItem('alertPreferences') : null;
      if (stored) {
        this.preferences = JSON.parse(stored);
      } else {
        this.preferences = this.getDefaultPreferences();
        if (canUseStorage) this.savePreferences();
      }
    } catch (error) {
  ...
  private savePreferences() {
    if (this.preferences) {
      if (typeof window !== 'undefined' && typeof window.localStorage !== 'undefined') {
        window.localStorage.setItem('alertPreferences', JSON.stringify(this.preferences));
      }
    }
  }
