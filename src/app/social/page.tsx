...
export default function SocialPage() {
  ...
  const openUserDetails = (user: SocialUser) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-900 dark:text-white">Enable Features</span>
          </div>
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-700 rounded-md p-3">
              <span className="text-sm text-gray-700 dark:text-gray-300">Copy Trade</span>
              <Switch checked={copyTradeEnabled} onCheckedChange={setCopyTradeEnabled} aria-label="Enable Copy Trade" />
            </div>
            <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-700 rounded-md p-3">
              <span className="text-sm text-gray-700 dark:text-gray-300">Follow Actions</span>
              <Switch checked={followEnabled} onCheckedChange={setFollowEnabled} aria-label="Enable Follow Actions" />
            </div>
          </div>
        </div>
        {/* Tabs */}
                ...
                return (
                  <div key={socialUser.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={() => openUserDetails(socialUser)}
                        className="w-full py-2 px-4 rounded-lg font-medium transition-colors bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 flex items-center justify-center space-x-2"
                      >
      ...
      <Dialog open={userModalOpen} onOpenChange={setUserModalOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedUser?.username} • {selectedUser ? formatNumber(selectedUser.followers) : ''} followers
            </DialogDescription>
          </DialogHeader>
          {selectedUser && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className={`font-semibold ${selectedUser.totalReturnPercent >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{selectedUser.totalReturnPercent >= 0 ? '+' : ''}{selectedUser.totalReturnPercent}%</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Win Rate</p>
                  <p className="font-semibold">{selectedUser.winRate}%</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Risk Score</p>
                  <p className="font-semibold">{selectedUser.riskScore}</p>
                </div>
                <div>
              ...
              {/* Enable/Disable Copy Trade */}
              <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                <div className="flex items-center space-x-2">
                  <Eye className="h-4 w-4 text-gray-600 dark:text-gray-400" />
                  <span className="text-sm text-gray-700 dark:text-gray-300">Enable Copy Trade</span>
                </div>
                <Switch
                  checked={copyTradeEnabled}
                  onCheckedChange={setCopyTradeEnabled}
                  aria-label="Enable Copy Trade"
                />
              </div>
