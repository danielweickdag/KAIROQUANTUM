...
export default function BrokerAccountSelector({
  ...
  const [sortBy, setSortBy] = useState<string>('portfolio');
          ...
          <div className="px-3 py-2 border-b border-gray-200 bg-gray-50">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div className="flex items-center space-x-2">
                <label className="text-xs text-gray-600">Broker</label>
                <select
                  className="flex-1 rounded-md border border-gray-300 bg-white px-2 py-1 text-xs focus:border-blue-500 focus:outline-none"
                  value={filterBroker}
                  onChange={(e) => setFilterBroker(e.target.value)}
                >
                  <option value="ALL">All</option>
                  {availableBrokers.map(bt => (
                    <option key={bt} value={bt}>{getBrokerDisplayName(bt)}</option>
                  ))}
                </select>
              </div>
              <div className="flex items-center space-x-2">
                ...
                >
                  ...
                  {availableTypes.map(t => (
                    <option key={t} value={t}>{getAccountTypeDisplay(t)}</option>
                  ))}
                </select>
              </div>
              <div className="flex items-center space-x-2">
                ...
                >
                  ...
                  <option value="buyingPower">Buying Power</option>
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <Link href="/brokers" className="text-xs text-blue-600 hover:underline">Connect Broker</Link>
                <Link href="/settings" className="text-xs text-gray-600 hover:underline">Manage</Link>
              </div>
            </div>
          </div>
