...
export default function TradingViewLanding() {
  ...
  const mapCryptoToSymbol = (sym: string): string => {
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {majorIndices.map((item) => (
                <Card
                  key={item.symbol}
                  className="market-widget cursor-pointer hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary"
                  role="button"
                  tabIndex={0}
                  onClick={() => openTradeModal(item.symbol)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      openTradeModal(item.symbol);
                    }
                  }}
                >
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <button
                        ...
                        aria-pressed={favorites.has(item.symbol)}
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      >
                        <Star className={`h-4 w-4 ${favorites.has(item.symbol) ? 'text-yellow-500' : 'text-muted-foreground'}`} />
                      </button>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {cryptoData.map((item) => (
                <Card
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => openTradeModal(mapCryptoToSymbol(item.symbol))}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      openTradeModal(mapCryptoToSymbol(item.symbol));
                    }
                  }}
                >
                  <CardContent className="p-4">
    ...
    <Dialog open={tradeModalOpen} onOpenChange={setTradeModalOpen}>
      <DialogContent className="sm:max-w-[900px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Review details and execute trades for the selected asset.
          </DialogDescription>
        </DialogHeader>
        <TradingPanel
