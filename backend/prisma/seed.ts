...
async function main() {
  ...
  // Admin credentials
  const adminEmail = 'danielw@blvckdlphn.com';
  const adminPassword = 'Mougouli05172023!!#$$!!*';
  ...
  const adminUser = await prisma.user.upsert({
    ...
    update: {
      ...
      bio: 'KAIRO Platform Administrator',
    },
    create: {
      email: adminEmail,
      username: 'blvckdlphn_admin',
  ...
  const demoUserPassword = 'Mougouli05172023!!#$$!!*';
  const demoPasswordHash = await bcrypt.hash(demoUserPassword, saltRounds);
  ...
  // Use upsert to create or update demo user
